...
const beforeUpload: UploadProps['beforeUpload'] = (rawFile: File) => {
  ...
  if (rawFile.size / 1024 / 1024 > 5) { // Limit size to 5MB
...
<template>
  <div class="lab-container">
      ...
      <el-tab-pane label="情绪色谱仪" name="colorEmotion">
         <el-card class="tool-panel">
            <el-row :gutter="30">
                 ...
                 <el-col :span="16">
                     <h4>2. 色彩情感分析结果</h4>
                     <div class="analysis-results" v-loading="isAnalyzingColor" element-loading-text="正在分析色彩情感...">
                                 ...
                                 <el-col :span="12">
                                     <h5>主要色彩</h5>
                                      <div class="dominant-colors">
                                        <span v-for="color in colorAnalysisResult.dominantColors" :key="color" :style="{ backgroundColor: color }" class="color-swatch"></span>
                                      </div>
                                     <h5>情感关键词</h5>
                                     <!-- TODO: Implement Word Cloud Component -->
                                     <div class="keyword-cloud">
                                         <el-tag v-for="word in colorAnalysisResult.keywords" :key="word" style="margin: 4px;">{{ word }}</el-tag>
                                     </div>
                                 </el-col>
                             </el-row>
                         </div>
      ...
      <el-tab-pane label="绘影工场" name="imageToVideo">
         <el-card class="tool-panel">
             <el-row :gutter="30">
                 ...
                 <el-col :span="16">
                     <h4>2. 生成结果</h4>
                     <div class="video-results" v-loading="isGeneratingVideo" element-loading-text="正在生成视频...">
    ...
    <el-dialog v-model="showVideoPlayerDialog" title="视频预览" width="70%" destroy-on-close>
        <video v-if="generatedVideoUrl" :src="generatedVideoUrl" controls autoplay style="width: 100%; max-height: 70vh;">
            您的浏览器不支持 Video 标签。
        </video>
    </el-dialog>
