...
import { ElMessage, ElDialog, ElButton, ElInput, ElTabs, ElTabPane, ElCard, ElRow, ElCol, ElTag, ElAvatar, ElIcon, ElTooltip, ElUpload, ElImage, ElEmpty } from 'element-plus'
...
const availableTags = ref(['AI教学', '课堂互动', '教学资源', '项目式学习', '新课标', '德育教育', '家校沟通']); // Example tags

// --- Feed State --- 
const activeTab = ref('latest');
const searchQuery = ref('');
const posts = ref([]); // Holds the list of posts
...
const submitPost = async () => {
    ...
    posts.value.unshift({
      ...
      attachments: newPostForm.attachments.map(f => ({ name: f.name, size: `${(f.size / 1024).toFixed(1)}KB` })),
...
const fetchPosts = async (loadMore = false) => {
    ...
    console.log(`Fetching posts: page=${currentPage.value}, tab=${activeTab.value}, search=${searchQuery.value}`);
    ...
    const newPosts = [
         ...
         { id: 100 + currentPage.value * 5 - 4, author: { name: '王老师', title: '特级教师', avatar: '/src/assets/teacher1.svg' }, createdAt: '2小时前', content: '分享一个关于《红楼梦》人物分析的教学设计...', images: [], attachments: [{ name: '教学设计.docx', size: '1.2MB' }], likes: 125, comments: 32, favorites: 58, isFeatured: true, tags:['教学设计', '语文'] },
         { id: 100 + currentPage.value * 5 - 3, author: { name: '李老师', title: '骨干教师', avatar: '/src/assets/teacher2.svg' }, createdAt: '5小时前', content: '推荐一个AI批改数学作业的工具... #AI助教', images: ['/src/assets/ai_tool.png'], attachments: [], likes: 88, comments: 15, favorites: 40, isFeatured: false, tags:['AI教学', '数学'] },
         { id: 100 + currentPage.value * 5 - 2, author: { name: '张老师', title: '青年教师', avatar: '/src/assets/teacher3.svg' }, createdAt: '昨天', content: '用「幻画实验室」的风格迁移功能，看看孩子们的创意大作！', images: ['/src/assets/artwork_styled1.jpg', '/src/assets/artwork_styled2.jpg'], attachments: [], likes: 210, comments: 45, favorites: 95, isFeatured: false, tags:['美术', 'AI创作'] },
         { id: 100 + currentPage.value * 5 - 1, author: { name: '赵老师', title: '教研组长', avatar: '/src/assets/teacher4.svg' }, createdAt: '2天前', content: '关于项目式学习的一些思考和实践案例分享。', images: [], attachments: [], likes: 150, comments: 28, favorites: 70, isFeatured: false, tags:['项目式学习'] },
         { id: 100 + currentPage.value * 5 - 0, author: { name: '孙老师', title: '信息技术教师', avatar: '/src/assets/teacher5.svg' }, createdAt: '3天前', content: '如何利用在线工具进行互动课堂测验？', images: [], attachments: [{ name: '工具对比.pdf', size: '800KB' }], likes: 95, comments: 12, favorites: 35, isFeatured: false, tags:['课堂互动', '在线工具'] }
    ];
  ...
  <div class="community-container">
        ...
        <div class="main-content">
          ...
          <el-card class="post-creator-card simple">
             ...
             <div class="creator-actions simple">
                  <el-button text @click="openPostDialog"><el-icon><Picture /></el-icon>图片</el-button>
                  <el-button text @click="openPostDialog"><el-icon><Paperclip /></el-icon>附件</el-button>
                  <el-button text @click="openPostDialog"><el-icon><CollectionTag /></el-icon>话题</el-button>
             </div>
          </el-card>
          ...
          <div class="post-feed" v-loading="loadingMore && posts.length === 0"> 
            <el-card class="post-card" v-for="(post, index) in posts" :key="post.id"> 
              ...
              <div class="post-content">
                ...
                <div class="post-tags" v-if="post.tags && post.tags.length">
                     <el-tag v-for="tag in post.tags" :key="tag" type="info" size="small" effect="plain"># {{ tag }}</el-tag>
                </div>
                <!-- Images -->
              ...
              <div class="post-actions">
                ...
                <span><el-icon><Star /></el-icon> {{ post.favorites }}</span>
            ...
            <el-empty v-if="!loadingMore && posts.length === 0" description="暂无相关帖子"></el-empty>
          </div>
        </div>
      </el-col>
      ...
      <el-col :xs="24" :sm="24" :md="24" :lg="6" :xl="6">
         <div class="right-sidebar">
           <!-- Recommended Users Card -->
            <el-card class="recommend-card">
             ...
             <div class="recommend-list">
               ...
               <el-empty v-if="!recommendedUsers.length" description="暂无推荐" :image-size="50"></el-empty>
             </div>
           </el-card>
           <!-- Hot Tags Card -->
           <el-card class="tags-card">
             ...
             <div class="tag-list">
               <el-tag v-for="tag in hotTags" :key="tag" class="hot-tag" effect="plain"># {{ tag }}</el-tag>
             </div>
    ...
    <el-dialog v-model="showPostDialog" title="发布新帖子" width="60%" top="8vh" destroy-on-close>
        ...
        <template #footer>
            ...
            <el-button type="primary" @click="submitPost" :loading="isPosting" :disabled="!newPostForm.content.trim()">
                {{ isPosting ? '发布中...' : '确认发布' }}
            </el-button>
            </span>
        </template>
