...
const generateAIPaper = async () => {
  ...
  // 如果没有描述，至少要有一些基本设置
  if (
    !generatorSettings.description &&
    (!generatorSettings.questionTypes.length || !generatorSettings.knowledgePoints.length)
  ) {
  ...
  try {
    ...
    // 构建查询参数
    const filters = {
      type: generatorSettings.questionTypes.length ? generatorSettings.questionTypes : [],
      difficulty: generatorSettings.difficulty || 'all',
      knowledgePoint: generatorSettings.knowledgePoints.length
        ? generatorSettings.knowledgePoints[0]
        : 'all',
    ...
    if (response.success && response.data.questions.length > 0) {
      ...
      if (props.paperPreviewRef) {
        const selected = props.paperPreviewRef.selectedQuestions;
      ...
      <el-form :model="generatorSettings" label-position="top">
        ...
        <div class="form-row">
          <el-form-item label="总题数">
            <el-input-number
              v-model="generatorSettings.totalQuestions"
              :min="1"
              :max="30"
            ></el-input-number>
          </el-form-item>
        ...
        <el-form-item label="题型分布">
          <el-checkbox-group v-model="generatorSettings.questionTypes">
            <el-checkbox v-for="type in questionTypeOptions" :key="type" :label="type">
              {{ type }}
            </el-checkbox>
          </el-checkbox-group>
        </el-form-item>
